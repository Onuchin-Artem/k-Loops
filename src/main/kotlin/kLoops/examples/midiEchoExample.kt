...
fun main() {
    ...
    loop("arp") {
        val baseChord = receiveParameter("chord") as Chord? ?: return@loop
    ...
    runWhenEvent("arpEcho", listOf("arp")) {
        ...
        if (midi is midi) {
            ...
            if (1 in 2) {
                ...
                if (midi.velocity > 0.07) triggerEvent("arp", midi.velocity - 0.03)
    ...
    loop("sample") {
        if (7 in 8) track(" sample").playAsync("b3", 3 o 4, 0.8)
        silence(1 o 1)
    }
